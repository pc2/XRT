...
rocky_package_list() {
    echo "collecting all packages for rocky OS ($VERSION, $MAJOR)"
    ROCY_LIST=(\
     boost-devel \
     boost-filesystem \
     boost-program-options \
...
install_pybind11()
{
    ...
    if [ $FLAVOR == "mariner" || $FLAVOR == "rocky"]; then
...
install()
{
    ...
    if [ $FLAVOR == "rocky" ]; then
        echo "Installing Rocky packages..."
        echo $ARCH
        #dnf install -y "${ROCY_LIST[@]}" --nobest
        dnf install -y "${ROCY_LIST[@]}"
        echo "exiting"
        exit 1
    fi
